...
export interface Facts {
  ...
  baseVersion: string;
  /** highest `vX.Y.Z` git tag, or null if none exist yet (catamorbius is not published to npm) */
...
export function evaluate(f: Facts): GateResult {
  ...
  // 2. strictly greater than the latest git tag (no npm registry — catamorbius is private)
  const latest = f.latestTag ? parse(f.latestTag) : null;
  if (latest) v(compare(to, latest) > 0, compare(to, latest) > 0 ? `greater than latest tag v${f.latestTag}` : `${f.version} is not greater than latest tag v${f.latestTag}`);
  ...
  // 4. changelog entry for this version, new in this PR, dated, with content
