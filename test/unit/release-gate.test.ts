...
const base: Facts = { version: "0.1.1", baseVersion: "0.1.0", latestTag: "0.1.0", changedFiles: ["src/a.ts"], changelog: log("0.1.1"), baseChangelog: log("0.1.0").replace(/## \[0\.1\.1\][\s\S]*?(?=## \[0\.1\.0\])/, ""), today: "2026-08-24" };
...
describe("release gate", () => {
  ...
  test("not greater than latest tag fails", () => { expect(failing({ latestTag: "0.1.1" })[0]).toMatch(/not greater than latest tag/); });
